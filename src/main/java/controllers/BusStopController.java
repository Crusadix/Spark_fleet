	...
	public BusStopController(final BusStopService busService) {
		...
		put("/stops", (req, res) -> {
			...
			if ((storageType != null) || (fuelType != null)) {
				return busService.createStation(Integer.valueOf(req.queryParams("id")), fuelType, storageType,
						req.queryParams("location"));
			} else {
				return busService.createBusStop(Integer.valueOf(req.queryParams("id")), req.queryParams("location"));
