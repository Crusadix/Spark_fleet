...
public class PassengerService {
	...
	// make a list of buses' passengers' destinations and nearby passengers' coordinates and return it
	public String[] buildPassengerWaypoints(VehicleInterface vehicle, String locationCoords, String origin, String destination) throws ApiException, InterruptedException, IOException {
		...
		for (int y = 0; y < waypointCoords.length; y++) {
			...
			else if(waitingPassengersPointer < orderedWaitingPassengersByDistance.size() && vehicle.getFreeSeats() > 0) {
	...
	private List<PassengerInterface> orderByDistance(List<PassengerInterface> waitingPassengers,
			String origin, String destination) throws ApiException, InterruptedException, IOException {
		FleetManager fleetManagement = FleetManager.getInstance();
		...
		for (int x = 0; x < waitingPassengers.size(); x++) {
			...
			double distanceDouble = distanceUtils.getDistanceMeters(step.startLocation.toString(), tempPassenger.getCurrentCoords());
		...
		for (int x = 0; x < waitingPassengers.size(); x++) {
			min = Collections.min(passengersByDistance.entrySet(),
